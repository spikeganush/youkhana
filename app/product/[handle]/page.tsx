        ...
        <div className='max-w-lg px-4 pb-16 pt-10 sm:px-6'>
          ...
          <div className='mt-6 lg:row-span-3 lg:mt-0'>
            ...
            <div className='space-y-2'>
              ...
              {(rentalProduct.rentalPrice.weekly || rentalProduct.rentalPrice.monthly) && (
              ...
              {rentalProduct.deposit && (
                <p className='text-sm text-gray-600 mt-2'>
                  Security Deposit: <span className='font-medium text-gray-900'>${rentalProduct.deposit.toFixed(2)}</span>
                </p>
              )}
            </div>
          </div>
          ...
          <div className='mt-4'>
            <p className='text-sm text-gray-600'>
              Availability: <span className='font-medium text-gray-900'>
                {rentalProduct.availableQuantity} of {rentalProduct.totalQuantity} available
              </span>
            </p>
          </div>
          ...
          <div className='py-10 lg:col-span-2 lg:col-start-1 lg:pb-8 lg:pr-8 lg:pt-6'>
            ...
            {/* Specifications */}
            {rentalProduct.specifications && Object.keys(rentalProduct.specifications).length > 0 && (
              <div className='mt-8'>
                <h3 className='text-lg font-medium text-gray-900 mb-3'>Specifications</h3>
                <dl className='space-y-2'>
                  {Object.entries(rentalProduct.specifications).map(([key, value]) => (
            ...
            {/* Rental Terms */}
            {(rentalProduct.minRentalDays || rentalProduct.maxRentalDays || rentalProduct.terms) && (
              <div className='mt-8'>
                <h3 className='text-lg font-medium text-gray-900 mb-3'>Rental Terms</h3>

                {(rentalProduct.minRentalDays || rentalProduct.maxRentalDays) && (
                  <p className='text-sm text-gray-600 mb-2'>
                    {rentalProduct.minRentalDays && `Minimum rental: ${rentalProduct.minRentalDays} days`}
                    {rentalProduct.minRentalDays && rentalProduct.maxRentalDays && ' • '}
                    {rentalProduct.maxRentalDays && `Maximum rental: ${rentalProduct.maxRentalDays} days`}
                  </p>
                )}
